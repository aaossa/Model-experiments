...
class CuratorNet2(nn.Module):
    ...
    def __init__(self, embedding, input_size=2048):
        ...
        # self.l2pool = ...
        self.selu_pu1 = nn.Linear(200 + 200 + 200, 1024)
        self.selu_pu2 = nn.Linear(1024, 1024)
        self.selu_pu3 = nn.Linear(1024, 200)
    ...
    def forward(self, profile, pi, ni):
        ...
        pool1 = self.maxpool(profile)
        pool2 = self.avgpool(profile)
        pool3 = torch.norm(profile, p="fro", dim=1).unsqueeze(1)
        profile = torch.cat(
            (pool1, pool2, pool3), dim=-1
        )
