...
def train_with_batch_samplers(
    ...
    pbar_train = tqdm(
        total=train_per_valid_times*len(batch_samplers["train"]),
        desc="Train",
        postfix={
            "last_acc": None,
            "last_loss": None,
    ...
    pbar_valid = tqdm(
        total=len(batch_samplers["validation"]),
        desc="Valid",
        postfix={
            "best_acc": None,
            "best_loss": None,
    ...
    for epoch in range(1, max_epochs + 1):
        ...
        for phase in ["train", "validation"]:
            ...
            # Iterate over data
            dataset = batch_samplers[phase].sampler.data_source
            loop_times = train_per_valid_times if phase == "train" else 1
            for _ in range(loop_times):
                for i_batch, data in enumerate(batch_samplers[phase]):
