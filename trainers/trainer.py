...
class Trainer:
    ...
    def __init__(
        ...
        self.writer_dst = os.path.join(writer_dir, f"{self.version}.tar")
        print(f">> Writer data stored at... {self.writer_dst}")
        # Create SummaryWriter instance
        self.summary_writer = SummaryWriter(self.writer_dst, flush_secs=20)
    ...
    def run(
        ...
        for epoch in range(1, max_epochs + 1):
            ...
            for phase in ["train", "validation"]:
                ...
                self.summary_writer.add_scalar(f"{phase} loss", epoch_loss, epoch)
                self.summary_writer.add_scalar(f"{phase} accuracy", epoch_acc, epoch)
